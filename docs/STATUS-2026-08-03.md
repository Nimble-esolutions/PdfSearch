...
Historical sequence: superseded docs/STATUS-2026-08-02.md as the next dated snapshot

# Operational status — 2026-08-03

> Historical snapshot: use [HANDOFF.md](HANDOFF.md) for current state, current
> blockers, and next actions. This page preserves the evidence available on
> 2026-08-03 and is not continuously updated.
...
| Real stage backup | Two manual stage points published; physical object reuse observed | Passed |
| Real stage recovery test | Isolated rehearsal succeeded with indexing ratio 1.0 | Passed |
| Stage runtime/readiness | `/readyz` reports the signed active generation and indexing ratio 1.0 | Passed |
| Operator workbench correction | Single readiness authority, scaled maintenance scope, and responsive 105-document fixture pass locally | Pending merge/deploy |

Stage backup remains a separately invoked recovery proof rather than a web
availability prerequisite. Automatic backup stays off unless explicitly
approved.
...
flowchart LR
  ...
  T --> F{"Stage :latest pull\nand readiness projection"}
  F -->|verified digest| R["Operator workbench\nand search smoke"]
```
...
The signed control pointer is authoritative. The deployed readiness endpoint
now reports this v3 evidence. The current operator-workbench branch removes the
remaining Dashboard-only legacy Vault projection so Dashboard and `/readyz`
cannot disagree.
...
Fresh controlled English and Marathi public searches then returned HTTP 200,
real-provider answers, and non-empty protected references; no sandbox marker
was present. The release in PR 176 additionally makes sandbox chat and test
embeddings fail closed in ordinary review, stage, and production runtimes, and
validates provider policy before reading the shared answer cache.

## Next stage release sequence

1. Make the operator-workbench PR green and merge it.
2. Publish/update the approved stage `:latest` channel and let Dokploy pull it
   for both web and maintenance; record the resolved actual image digest, OCI
   revision, packaged source, mounts, policy posture, and health.
...
5. Re-run a manual stage backup and isolated disposable restore when recovery
   code or data contracts change. It is not required for every UI-only release.
