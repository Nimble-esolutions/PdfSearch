...
from .utils import (
    ...
    TOP_K_CHUNKS,
    MAX_CONTEXT_WORDS,
    generate_gpt_answer,process_pdf_pipeline
...
def dashboard(request, folder_id=None):
    ...
    if role not in ["admin", "superadmin"]:
        return redirect("ask-page")  # non-admins redirected
    ...
    if folder_id:
        ...
        if request.method == "POST":
            ...
            if form.is_valid():
                ...
                # 🚀 ASYNC PIPELINE
                process_pdf_pipeline.delay(pdf.id)
    ...
    folders = Folder.objects.annotate(pdf_count=Count("files", distinct=True))
...
def search_query(request):
    if request.method == "GET":
        welcome_message = (
            "🙏 नमस्कार ! मी तुमचा AI सहकारी आहे. "
            "माझ्याकडे असलेल्या दस्तऐवजांच्या मधून मी उत्तर देईन."
        )
    ...
    if request.method == "POST":
        try:
            ...
            # if subject == "housing":
            #     answer, references = generate_housing_answer(question)

            if subject == "housing":
                result = generate_housing_answer(question)
...
def search_query(request):
    if request.method == "GET":
        welcome_message = (
            "🙏 नमस्कार ! मी तुमचा AI सहकारी आहे. माझ्याकडे असलेल्या दस्तऐवजांच्या मधून मी उत्तर देईन. "
            "तुमचे प्रश्न गृहनिर्माण विषयाशी निगडित असतील तर प्रश्न विचारतांना "
            "'Housing/गृहनिर्माण' हा विषय निवडा म्हणजे मी अचूक उत्तर देऊ शकेन."
        )
...
def search_query27Feb2026(request):
    if request.method == "GET":
        welcome_message = (
            "🙏 नमस्कार ! मी तुमचा AI सहकारी आहे. माझ्याकडे असलेल्या दस्तऐवजांच्या मधून मी उत्तर देईन. "
            "तुमचे प्रश्न गृहनिर्माण विषयाशी निगडित असतील तर प्रश्न विचारतांना "
