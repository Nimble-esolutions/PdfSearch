...
print("📦 faiss_utils.py loaded")

# ================= CONFIG =================

BASE_FAISS_DIR = os.path.join("media", "faiss")
os.makedirs(BASE_FAISS_DIR, exist_ok=True)

# ================= PATH HELPERS =================

def get_index_path(pdf_id: int) -> str:
    path = os.path.join(BASE_FAISS_DIR, f"{pdf_id}.index")
    print(f"📁 Index Path: {path}")
    return path


def get_meta_path(pdf_id: int) -> str:
    path = os.path.join(BASE_FAISS_DIR, f"{pdf_id}.meta.json")
    print(f"📁 Meta Path: {path}")
    return path

# ================= SAVE =================
def save_faiss_index(index: faiss.Index, pdf_id: int, metadata: dict):
    print(f"💾 Saving FAISS index for PDF ID={pdf_id}")

    index_path = get_index_path(pdf_id)
    meta_path = get_meta_path(pdf_id)
...
def load_faiss_index(pdf_id: int) -> Optional[Tuple[faiss.Index, dict]]:
    print(f"📂 Loading FAISS for PDF ID={pdf_id}")
    ...
    if not os.path.exists(meta_path):
        print("❌ FAISS metadata not found")
        return None
...
def delete_faiss_index(pdf_id: int):
    print(f"🗑️ Deleting FAISS for PDF ID={pdf_id}")
...
def build_faiss_for_pdf(pdf):
    ...
    chunks = pdf.chunks  # assuming stored in DB or extracted earlier
    ...
    # Save chunks JSON
    json_path = os.path.join(faiss_dir, f"{pdf.id}.json")
    with open(json_path, "w", encoding="utf-8") as f:
        json.dump(chunks, f, ensure_ascii=False)

def embedding_functions(texts: List[str], model: str = "text-embedding-3-small") -> List[List[float]]:
    """
    Convert a list of strings into embeddings.

    Args:
        texts (List[str]): List of texts to embed
        model (str): Embedding model to use. Default: OpenAI 'text-embedding-3-small'
    ...
    if client is None:
        raise ValueError("OpenAI client is not initialized. Set OPENAI_API_KEY in your environment.")
    ...
    # Convert to numpy arrays if needed (FAISS usually prefers float32)
    embeddings = [np.array(vec, dtype=np.float32).tolist() for vec in embeddings]
...
LEGAL_CONTEXT_WORDS = [
    "document", "act", "law", "legal", "नियम", "कायदा", "दस्तऐवज"
]
# ------------------ Helpers ------------------
...
def classify_query(query: str) -> str:
    ...
    print("\n🔍 [QueryClassifier] Classifying query:", query)

    # 1️⃣ Exact legal query
    for pattern in EXACT_LEGAL_PATTERNS:
        if re.search(pattern, q):
            print("✅ Classified as EXACT_LEGAL | Pattern:", pattern)
            return "EXACT_LEGAL"

    # 2️⃣ Informative legal query
    for word in INFORMATIVE_KEYWORDS:
        if word in q:
            print("✅ Classified as LEGAL_INFORMATIVE | Keyword:", word)
            return "LEGAL_INFORMATIVE"

    # 3️⃣ Contextual legal query
    for word in LEGAL_CONTEXT_WORDS:
        if word in q:
            print("✅ Classified as LEGAL_CONTEXTUAL | Word:", word)
...
def build_strict_extraction_prompt(question: str, context: str, lang: str) -> str:
...
def faiss_index_path_for_folder(folder_id: int) -> str:
    return os.path.join(BASE_DIR, f"faiss_folder_{folder_id}.index")


# def build_or_load_faiss_index_for_folder(folder: Folder) -> Tuple[Optional[faiss.Index], List[str]]:
def build_or_load_faiss_index_for_folder(folder_id: int):
    """
    ...
    if _HAS_FAISS:
        idx_path = faiss_index_path_for_folder(folder_id)
        try:
...
# ------------------ GPT answer ------------------
def generate_gpt_answer(user_question: str, context: str, references=None, max_words: int = 500) -> str:
    ...
    if not context or len(context.strip()) < 100:
        print("❌ Context empty or too small")
        return "⚠️ No sufficient document context found."
    ...
    if query_type == "EXACT_LEGAL":
        print("🚨 Using STRICT extraction prompt")
        prompt = build_strict_extraction_prompt(user_question, context, lang)
    else:
        print("📘 Using GENERAL prompt")
        prompt = build_general_prompt(user_question, context, lang)

    # Add references if available
    if references:
        print("📚 Adding references to prompt")
        refs_text = "\n".join(
    ...
    try:
        response = client.chat.completions.create(
            ...
            messages=[
                {"role": "system", "content": "You are a legal domain assistant."},
        ...
        traceback.print_exc()
        return "⚠️ Couldn't generate answer right now."
